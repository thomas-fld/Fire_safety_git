...
cs_user_1d_wall_thermal(int iappel)
{
   ...
   *                       m   (eshing and wall temperature reinitialization)
  ...
  if (iappel == 1 || iappel == 2) {
    ...
    // Get Walls boundary faces
    cs_selector_get_b_face_num_list("Walls_All", &nlelt, lstelt);
  ...
  if (iappel == 2) {
    for (ii = 0 ; ii < wall_thermal->nfpt1d ; ii++) {
      wall_thermal->local_models[ii].nppt1d = 10;
      wall_thermal->local_models[ii].eppt1d = 0.05;
    ...
    // TODO multicouche
    //
    // FIXME coherence proprietes vs epaisseur
      ifac = wall_thermal->ifpt1d[ii] - 1;
      if (cdgfbo[ifac][2] >= 3.94) {
        wall_thermal->local_models[ii].eppt1d = 0.35;
      /* Floor and walls : 300 mm of concrete (2430 kg/m3, 736 J/kg/K, 1.500 W/m/K) */
      }
      else if (cdgfbo[ifac][2] <= 0.02) {
        wall_thermal->local_models[ii].eppt1d = 0.3;
      }
      else {
        wall_thermal->local_models[ii].eppt1d = 0.3;
      }
  ...
  if (iappel == 3) {
    for (ii = 0 ; ii < wall_thermal->nfpt1d ; ii++) {
      ...
      wall_thermal->local_models[ii].hept1d = 6.;
      /* Physical parameters */
      ifac = wall_thermal->ifpt1d[ii] - 1;
      /* Ceiling : 50 mm of thermipan (140 kg/m3, 840 J/kg/K, 0.102 W/m/K)
                + 300 mm of concrete (2430 kg/m3, 736 J/kg/K, 1.500 W/m/K) */
      if (cdgfbo[ifac][2] >= 3.94) {
        wall_thermal->local_models[ii].xlmbt1 = 0.5;
        wall_thermal->local_models[ii].rcpt1d = 2102.*736.; //140.*840.;
      /* Floor and walls : 300 mm of concrete (2430 kg/m3, 880 J/kg/K, 2.2500 W/m/K) */
      }
      else if (cdgfbo[ifac][2] <= 0.02) {
        wall_thermal->local_models[ii].xlmbt1 = 1.5;
        wall_thermal->local_models[ii].rcpt1d = 2430.*880.;
      }
      else {
        wall_thermal->local_models[ii].xlmbt1 = 1.5;
        wall_thermal->local_models[ii].rcpt1d = 2430.*880.;
